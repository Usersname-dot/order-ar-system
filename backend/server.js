...
const pool = mysql.createPool({
  ...
  database: process.env.DB_NAME || 'order_ar_system',
...
async function seedData() {
  ...
  try {
    ...
    for (const c of customers) {
      await conn.execute('INSERT INTO customers (id, customerName, phone, wechat, companyName, address, settlementMethod, creditLimit, creditLevel, region, billingCompany, taxNumber, bankName, bankAccount, taxLevel) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', c);
    }
    ...
    for (const p of products) {
      await conn.execute('INSERT INTO products (id, productCode, brand, model, guidePrice, category, image, correspondingModel, remark) VALUES (?,?,?,?,?,?,?,?,?)', p);
    }
    ...
    for (const o of orders) {
      await conn.execute('INSERT INTO orders (id, orderNo, customerId, salesperson, date, warehouse, settlementMethod, items, status) VALUES (?,?,?,?,?,?,?,?,?)', o);
    }
    ...
    for (const r of returns) {
      await conn.execute('INSERT INTO `returns` (id, returnNo, customerId, salesperson, date, items, status) VALUES (?,?,?,?,?,?,?)', r);
    }
    ...
    for (const p of productions) {
      await conn.execute('INSERT INTO productions (id, orderNo, brand, model, quantity, unit, correspondingModel, workGroup, completeDate, remark, status) VALUES (?,?,?,?,?,?,?,?,?,?,?)', p);
    }
    ...
    for (const w of warehouse) {
      await conn.execute('INSERT INTO warehouse (id, orderNo, brand, model, quantity, unit, correspondingModel, completeDate, remark, status) VALUES (?,?,?,?,?,?,?,?,?)', w);
    }
    ...
    for (const s of shipments) {
      await conn.execute('INSERT INTO shipments (id, orderNo, brand, model, quantity, unit, shipDate, trackingNo, freight, trackingPhoto, status) VALUES (?,?,?,?,?,?,?,?,?,?,?)', s);
    }
    ...
    for (const p of payments) {
      await conn.execute('INSERT INTO payments (id, customerId, paymentDate, settlementMethod, amount, taxRate, tax, total, remittanceUnit) VALUES (?,?,?,?,?,?,?,?,?)', p);
    }
    ...
    for (const d of departments) {
      await conn.execute('INSERT INTO departments (id, name, `desc`, permissions) VALUES (?,?,?,?)', d);
    }
    ...
    for (const u of users) {
      await conn.execute('INSERT INTO users (username, password, name, deptId, deptName, role, phone, email, securityQuestion, securityAnswer) VALUES (?,?,?,?,?,?,?,?,?,?)', u);
    }
    ...
    for (const [name, value] of seqs) {
      await conn.execute('INSERT INTO seq (name, value) VALUES (?,?)', [name, value]);
    }
...
app.get('/api/all', async (req, res) => {
  try {
    ...
    // seq 表转为对象
    const [seqRows] = await pool.execute('SELECT * FROM seq');
    result.seq = {};
...
app.post('/api/:table', async (req, res) => {
  ...
  try {
    ...
    if (data.id === undefined || data.id === '') {
      ...
      const [seqRows] = await pool.execute('SELECT value FROM seq WHERE name = ?', [seqKey]);
      let nextVal = 1;
      if (seqRows.length > 0) {
        nextVal = seqRows[0].value;
        await pool.execute('UPDATE seq SET value = value + 1 WHERE name = ?', [seqKey]);
      } else {
        await pool.execute('INSERT INTO seq (name, value) VALUES (?, ?)', [seqKey, 2]);
      }
        ...
        if (!isNaN(num)) {
          ...
          if (seqRows.length > 0) {
            if (seqRows[0].value <= num) {
              await pool.execute('UPDATE seq SET value = ? WHERE name = ?', [num + 1, seqKey]);
            }
          } else {
            await pool.execute('INSERT INTO seq (name, value) VALUES (?, ?)', [seqKey, num + 1]);
          }
        }
      }
    }
...
app.put('/api/auth/update-user', async (req, res) => {
  ...
  try {
    ...
    if (updates.username && updates.username !== username) {
      const [existing] = await pool.execute('SELECT username FROM users WHERE username = ?', [updates.username]);
    ...
    if (setParts.length > 0) {
      ...
      await pool.execute(`UPDATE users SET ${setParts.join(', ')} WHERE username = ?`, values);
    }

    // 返回更新后的用户
    const newUsername = updates.username || username;
    const [rows] = await pool.execute('SELECT * FROM users WHERE username = ?', [newUsername]);
    if (rows.length > 0) {
      const user = processRow(rows[0], TABLE_CONFIG.users);
...
app.post('/api/departments/create', async (req, res) => {
  ...
  try {
    ...
    // 获取下一个部门ID
    const [maxRows] = await conn.execute('SELECT MAX(id) as maxId FROM departments');
...
app.put('/api/departments/:id/permissions', async (req, res) => {
  ...
  try {
    await pool.execute('UPDATE departments SET permissions = ? WHERE id = ?', [JSON.stringify(permissions), parseInt(id)]);
    res.json({ success: true });
  } catch(e) {
...
app.post('/api/import', async (req, res) => {
  ...
  try {
    ...
    await conn.execute('DELETE FROM seq');
    ...
    for (const table of VALID_TABLES) {
      ...
      for (const item of data[table]) {
        ...
        if (cols.length > 0) {
          await conn.execute(`INSERT INTO \`${table}\` (${colNames}) VALUES (${placeholders})`, values);
        }
      }
    }
